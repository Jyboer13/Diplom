...
export class ManagerDBService {
  constructor(
    @InjectModel(managerDBModel)
    private readonly candidateModel: typeof managerDBModel
  ) {}

  public async findAll(where?: WhereOptions): Promise<managerDBModel[]> {
    return this.candidateModel.findAll( { where } );
  }

  public async findOne(where: WhereOptions): Promise<managerDBModel> {
    return this.candidateModel.findOne( { where } );
  }

  public async create(data: any): Promise<managerDBModel>{
    return this.candidateModel.create(data)
  }

  public async update(where: WhereOptions, data: any): Promise<Array<number | managerDBModel[]>>{
